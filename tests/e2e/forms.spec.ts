...
for (const formCase of formCases) {
  test(`${formCase.name}: required validation blocks an empty submission`, async ({ page }) => {
    let apiRequests = 0;
  ...
  test(`${formCase.name}: mocked successful submission reaches the success state`, async ({
    ...
    const requestPromise = page.waitForRequest((request) => {
      return (
        request.method() === "POST" && new URL(request.url()).pathname === formCase.endpoint
      );
    });
  ...
  test(`${formCase.name}: privacy link is present and correctly linked`, async ({ page }) => {
    await page.goto(formCase.route);
    const form = page.locator(formCase.formSelector).first();
    const privacyLink = form.getByRole("link", { name: "Privacy Policy" }).first();
...
test("main contact form shows a server error without losing the page", async ({ page }) => {
...
test("floating contact form validates and submits safely with a mocked API", async ({ page }) => {
  ...
  await expect(dialog.getByText(/confirmation email is on its way/i)).toBeVisible();
});
